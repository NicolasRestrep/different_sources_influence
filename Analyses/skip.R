...
wave1 <-  wave1 %>% 
  mutate(actskip = case_when(actskip == 1 ~ 0, 
                             actskip == 2 ~ 1, 
                             actskip == 3 ~ 1, 
                             actskip == 4 ~ 1, 
                             actskip == 5 ~ 1)) 
...
wave2 <-  wave2 %>% 
  mutate(actskipb = case_when(actskipb == 1 ~ 0, 
                              actskipb == 2 ~ 1, 
                              actskipb == 3 ~ 1, 
                              actskipb == 4 ~ 1, 
                              actskipb == 5 ~ 1)) 
...
wave3 <-  wave3 %>% 
  mutate(actskipc = case_when(actskipc == 1 ~ 0, 
                              actskipc == 2 ~ 1, 
                              actskipc == 3 ~ 1, 
                              actskipc == 4 ~ 1, 
                              actskipc == 5 ~ 1))
...
wave4 <-  wave4 %>% 
  mutate(actskipd = case_when(actskipd == 1 ~ 0, 
                              actskipd == 2 ~ 1, 
                              actskipd == 3 ~ 1, 
                              actskipd == 4 ~ 1, 
                              actskipd == 5 ~ 1)) 
...
alc_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, actskip) %>% 
...
alc_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actskipb) %>% 
...
alc_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actskipc) %>% 
...
alc_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actskipd) %>% 
...
b1 <- brm(actskip ~ 1 + friends_z + wave + popular_z + (1 +  wave | id), 
          family = bernoulli, 
          data = dfl_scaled, 
