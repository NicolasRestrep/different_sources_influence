...
wave1 <- wave1 %>% 
  mutate(actsport = case_when(actsport == 1 ~ 5,
                              actsport == 2 ~ 4, 
                              actsport == 3 ~ 3, 
                              actsport == 4 ~ 2, 
                              actsport == 5 ~ 1))
...
wave2 <- wave2 %>% 
  mutate(actsporb = case_when(actsporb == 1 ~ 5,
                              actsporb == 2 ~ 4, 
                              actsporb == 3 ~ 3, 
                              actsporb == 4 ~ 2, 
                              actsporb == 5 ~ 1))
...
wave3 <- wave3 %>% 
  mutate(actsporc = case_when(actsporc == 1 ~ 5,
                              actsporc == 2 ~ 4, 
                              actsporc == 3 ~ 3, 
                              actsporc == 4 ~ 2, 
                              actsporc == 5 ~ 1))
...
wave4 <- wave4 %>% 
  mutate(actspord = case_when(actspord == 1 ~ 5,
                              actspord == 2 ~ 4, 
                              actspord == 3 ~ 3, 
                              actspord == 4 ~ 2, 
                              actspord == 5 ~ 1))
...
alc_scores_w1 <- map_df(list_schools, get_scores, df = wave1, dv = 'actsport')
...
alc_scores_w2 <- map_df(list_schools_w2, get_scores, df = wave2, dv = 'actsporb')

# Get scores for wave 3 and 4 
list_schools_w3 <- unique(wave3$schoolnr)
list_schools_w4 <- unique(wave4$schoolnr)
alc_scores_w3 <- map_df(list_schools_w3, get_scores, df = wave3, dv = 'actsporc')
alc_scores_w4 <- map_df(list_schools_w4, get_scores, df = wave4, dv = 'actspord')
...
cl_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, actsport) %>% 
  rename(friends_w1 = friends_score, 
         popular_w1 = popular_score, 
         actsport_w1 = actsport)

cl_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actsporb) %>% 
  rename(friends_w2 = friends_score, 
         popular_w2 = popular_score, 
         actsport_w2 = actsporb)

cl_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actsporc) %>% 
  rename(friends_w3 = friends_score, 
         popular_w3 = popular_score, 
         actsport_w3 = actsporc)

cl_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actspord) %>% 
  rename(friends_w4 = friends_score, 
         popular_w4 = popular_score, 
         actsport_w4 = actspord)
...
write_csv(dfl_scaled,"Data/sport_std.csv")

b1 <- brm(actsport ~  1 + friends_z + wave + popular_z + (1 +  wave | id), 
      data = dfl_scaled, 
      family = cumulative, 
      ...
      warmup = 1000, 
      cores = 6, chains = 2)
saveRDS(b1, "Analyses/sport_cumulative.rds")
