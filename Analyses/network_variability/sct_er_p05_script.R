...
diff_inf_sim <-
  function(turns,
    ...
    for (r in 1:rounds) {
      ...
      # Initial values
      beh_matrix[, 1] <-
        rbinom(n = n_agents, size = 1, prob = init_prob)
      for (j in 2:turns) {
        for (i in 1:n_agents) {
          ties <- which(net_mat[i, ] > 0)
          ...
          if (is_empty(ties)) {
            friends_score <- ind_weight[1]
          } else {
            friends_score <-
              sum(beh_matrix[ties, j - 1] * net_mat[i, ties] / sum(net_mat[i, ]))
          }
          if (is.nan(ind_weight[1])) {
            ind_weight[1] <- 1
          }
          prSmoke <-
            (1 - socinf) * ind_weight[1] + socinf * ((1 - g) * popular_score + g *
                                                       friends_score)
          beh_matrix[i, j] <-
            ifelse(rbernoulli(n = 1, p = prSmoke) == TRUE, 1, 0)
        }
      }
...
#### Scot Eigen Random ####
network <-
  readRDS("~/different_sources_influence/Analyses/network_variability/scot_eigen_rand.rds")
...
d5 <-
  map2(
    ...
    turns = 500,
    init_prob = 0.5,
    n_agents = length(V(network)),
    rounds = 25
  )
end <- Sys.time()
end - start

saveRDS(d5, "~/different_sources_influence/Analyses/network_variability/sct_er_p05.rds")