...
wave1 <-  wave1 %>% 
  mutate(actfare = case_when(actfare == 1 ~ 0, 
                              actfare == 2 ~ 1, 
                              actfare == 3 ~ 1, 
                              actfare == 4 ~ 1, 
                              actfare == 5 ~ 1)) 
...
wave2 <-  wave2 %>% 
  mutate(actfareb = case_when(actfareb == 1 ~ 0, 
                              actfareb == 2 ~ 1, 
                              actfareb == 3 ~ 1, 
                              actfareb == 4 ~ 1, 
                              actfareb == 5 ~ 1)) 
...
wave3 <-  wave3 %>% 
  mutate(actfarec = case_when(actfarec == 1 ~ 0, 
                              actfarec == 2 ~ 1, 
                              actfarec == 3 ~ 1, 
                              actfarec == 4 ~ 1, 
                              actfarec == 5 ~ 1))
...
wave4 <-  wave4 %>% 
  mutate(actfared = case_when(actfared == 1 ~ 0, 
                              actfared == 2 ~ 1, 
                              actfared == 3 ~ 1, 
                              actfared == 4 ~ 1, 
                              actfared == 5 ~ 1)) 
...
alc_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, actfare) %>% 
...
alc_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actfareb) %>% 
...
alc_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actfarec) %>% 
...
alc_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actfared) %>% 
...
write.csv(dfl_scaled, "Data/fare_std.csv")

b1 <- brm(actfare ~ 1 + friends_z + wave + popular_z + (1 +  wave | id), 
          family = bernoulli, 
          data = dfl_scaled, 
...
saveRDS(b1, "Analyses/fare_model.rds")
