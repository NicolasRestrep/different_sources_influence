...
wave1 <-  wave1 %>% 
  mutate(actbike = case_when(actbike == 1 ~ 0, 
                             actbike == 2 ~ 1, 
                             actbike == 3 ~ 1, 
                             actbike == 4 ~ 1, 
                             actbike == 5 ~ 1)) 
...
wave2 <-  wave2 %>% 
  mutate(actbikeb = case_when(actbikeb == 1 ~ 0, 
                              actbikeb == 2 ~ 1, 
                              actbikeb == 3 ~ 1, 
                              actbikeb == 4 ~ 1, 
                              actbikeb == 5 ~ 1)) 
...
wave3 <-  wave3 %>% 
  mutate(actbikec = case_when(actbikec == 1 ~ 0, 
                              actbikec == 2 ~ 1, 
                              actbikec == 3 ~ 1, 
                              actbikec == 4 ~ 1, 
                              actbikec == 5 ~ 1))
...
wave4 <-  wave4 %>% 
  mutate(actbiked = case_when(actbiked == 1 ~ 0, 
                              actbiked == 2 ~ 1, 
                              actbiked == 3 ~ 1, 
                              actbiked == 4 ~ 1, 
                              actbiked == 5 ~ 1)) 
...
alc_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, actbike) %>% 
...
alc_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actbikeb) %>% 
...
alc_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actbikec) %>% 
...
alc_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actbiked) %>% 
...
write.csv(dfl_scaled, "Data/bike_std.csv")

b1 <- brm(actbike ~ 1 + friends_z + wave + popular_z + (1 +  wave | id), 
          family = bernoulli, 
          data = dfl_scaled, 
...
saveRDS(b1, "Analyses/bike_model.rds")
