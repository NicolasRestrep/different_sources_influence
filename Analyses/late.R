...
wave1 <-  wave1 %>% 
  mutate(actlate = case_when(actlate == 1 ~ 0, 
                              actlate == 2 ~ 1, 
                              actlate == 3 ~ 1, 
                              actlate == 4 ~ 1, 
                              actlate == 5 ~ 1)) 
...
wave2 <-  wave2 %>% 
  mutate(actlateb = case_when(actlateb == 1 ~ 0, 
                              actlateb == 2 ~ 1, 
                              actlateb == 3 ~ 1, 
                              actlateb == 4 ~ 1, 
                              actlateb == 5 ~ 1)) 
...
wave3 <-  wave3 %>% 
  mutate(actlatec = case_when(actlatec == 1 ~ 0, 
                              actlatec == 2 ~ 1, 
                              actlatec == 3 ~ 1, 
                              actlatec == 4 ~ 1, 
                              actlatec == 5 ~ 1))
...
wave4 <-  wave4 %>% 
  mutate(actlated = case_when(actlated == 1 ~ 0, 
                              actlated == 2 ~ 1, 
                              actlated == 3 ~ 1, 
                              actlated == 4 ~ 1, 
                              actlated == 5 ~ 1)) 
...
alc_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, actlate) %>% 
...
alc_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actlateb) %>% 
...
alc_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actlatec) %>% 
...
alc_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actlated) %>% 
...
b1 <- glmer(actlate ~  1 + friends_z + wave + popular_z + (1 +  wave | id), 
           data = dfl_scaled, 
           family = "binomial")
