...
wave1 <-  wave1 %>% 
  mutate(actfight = case_when(actfight == 1 ~ 0, 
                              actfight == 2 ~ 1, 
                              actfight == 3 ~ 1, 
                              actfight == 4 ~ 1, 
                              actfight == 5 ~ 1)) 
...
wave2 <-  wave2 %>% 
  mutate(actfighb = case_when(actfighb == 1 ~ 0, 
                              actfighb == 2 ~ 1, 
                              actfighb == 3 ~ 1, 
                              actfighb == 4 ~ 1, 
                              actfighb == 5 ~ 1)) 
...
wave3 <-  wave3 %>% 
  mutate(actfighc = case_when(actfighc == 1 ~ 0, 
                              actfighc == 2 ~ 1, 
                              actfighc == 3 ~ 1, 
                              actfighc == 4 ~ 1, 
                              actfighc == 5 ~ 1))
...
wave4 <-  wave4 %>% 
  mutate(actfighd = case_when(actfighd == 1 ~ 0, 
                              actfighd == 2 ~ 1, 
                              actfighd == 3 ~ 1, 
                              actfighd == 4 ~ 1, 
                              actfighd == 5 ~ 1)) 
...
alc_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, actfight) %>% 
  rename(friends_w1 = friends_score, 
         popular_w1 = popular_score, 
         actfight_w1 = actfight)

alc_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actfighb) %>% 
  rename(friends_w2 = friends_score, 
         popular_w2 = popular_score, 
         actfight_w2 = actfighb)

alc_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actfighc) %>% 
  rename(friends_w3 = friends_score, 
         popular_w3 = popular_score, 
         actfight_w3 = actfighc)

alc_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, actfighd) %>% 
  rename(friends_w4 = friends_score, 
         popular_w4 = popular_score, 
         actfight_w4 = actfighd)
...
b1 <- glmer(actfight ~  1 + friends_z + wave + popular_z + (1 +  wave | id), 
            data = dfl_scaled, 
            family = "binomial")
