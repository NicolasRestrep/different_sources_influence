...
alc_scores_w1 <- map_df(list_schools, get_scores, df = wave1, dv = 'trust1')
...
alc_scores_w2 <- map_df(list_schools_w2, get_scores, df = wave2, dv = 'trust1b')

# Get scores for wave 3 and 4 
list_schools_w3 <- unique(wave3$schoolnr)
list_schools_w4 <- unique(wave4$schoolnr)
alc_scores_w3 <- map_df(list_schools_w3, get_scores, df = wave3, dv = 'trust1c')
alc_scores_w4 <- map_df(list_schools_w4, get_scores, df = wave4, dv = 'trust1d')
...
cl_w1 <- df1 %>% 
  ...
  select(id,friends_score, popular_score, trust1) %>% 
...
cl_w2 <- df2 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, trust1b) %>% 
...
cl_w3 <- df3 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, trust1c) %>% 
...
cl_w4 <- df4 %>% 
  filter(id %in% ids_all_waves) %>% 
  select(id,friends_score, popular_score, trust1d) %>% 
